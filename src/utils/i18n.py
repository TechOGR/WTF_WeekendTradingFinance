"""
Módulo de internacionalización (i18n) para W-T-F Trading Manager.
Traducciones completas de todos los textos de la interfaz.
"""

from src.version import APP_VERSION

# Diccionario de traducciones
TRANSLATIONS = {
    "es": {
        # Títulos de ventanas
        "app_title": f"W-T-F (Weekend Trading Finance) Trading Manager v{APP_VERSION}",
        "capital_dialog_title": "Establecer Capital Inicial",
        "export_dialog_title": "Exportar Datos",
        "about_title": "Acerca de W-T-F Trading Manager",
        "instructions_title": "Instrucciones de Uso",
        "save_week_title": "Guardar Semana",
        "load_week_title": "Cargar Semana",
        
        # Menú principal
        "menu_file": "📁 Archivo",
        "menu_view": "👁️ Vista",
        "menu_assistant": "🧭 Asistente",
        "menu_export": "📊 Exportar",
        "menu_help": "❓ Ayuda",
        "menu_language": "🌍 Idioma",
        
        # Acciones de menú
        "save_week": "💾 Guardar Semana",
        "load_week": "📂 Cargar Semana",
        "load_from_db": "🗄️ Cargar desde Base de Datos",
        "set_capital": "💰 Establecer Capital Inicial",
        "exit": "🚪 Salir",
        "dark_mode": "🌙 Modo Oscuro",
        "toggle_legend": "👁️ Mostrar leyenda",
        "daily_advice": "📌 Mostrar consejo del día",
        "weekly_summary": "🗓️ Resumen semanal",
        "start_new_week_reset": "🆕 Empezar nueva semana (reiniciar datos)",
        "export_excel": "📈 Exportar a Excel",
        "export_csv": "📋 Exportar a CSV",
        "export_json": "📄 Exportar a JSON",
        "about": "ℹ️ Acerca de",
        "instructions": "📖 Instrucciones",
        "capital_edit_mode": "💹 Modo edición por capital",
        
        # Descripciones (StatusTip) del menú
        "status_save_week": "Guardar datos de la semana actual",
        "status_load_week": "Cargar datos desde archivo",
        "status_load_db": "Cargar datos guardados en la base de datos",
        "status_set_capital": "Configurar el capital inicial de la semana",
        "status_exit": "Salir de la aplicación",
        "status_dark_mode": "Activar/desactivar modo oscuro",
        "status_toggle_legend": "Mostrar u ocultar la leyenda del gráfico",
        "status_daily_advice": "Ver recomendaciones según el día actual",
        "status_weekly_summary": "Mostrar resumen con sugerencia de retiro y reinversión",
        "status_start_new_week_reset": "Crear semana nueva con datos en cero y capital actualizado",
        "status_export_excel": "Exportar datos a formato Excel (.xlsx)",
        "status_export_csv": "Exportar datos a formato CSV",
        "status_export_json": "Exportar datos a formato JSON",
        "status_about": "Información sobre la aplicación",
        "status_instructions": "Ver instrucciones de uso",
        "status_capital_edit_mode": "Editar el monto por capital inicial/actual",
        
        # Idiomas
        "spanish": "Español",
        "english": "English",
        
        # Botones y etiquetas generales
        "accept": "Aceptar",
        "cancel": "Cancelar",
        "save": "Guardar",
        "close": "Cerrar",
        "yes": "Sí",
        "no": "No",
        
        # Diálogo de capital
        "capital_label": "Capital Inicial:",
        "capital_tooltip": "Ingrese el capital inicial para la semana",
        "capital_placeholder": "Ej: 100.00",
        "day_capital_dialog_title": "Ingresar capital del día",
        "initial_capital": "Capital inicial",
        "current_capital": "Capital actual",
        
        # Tabla de trading
        "day_column": "Día",
        "type_column": "Tipo",
        "comments_column": "Comentarios",
        "amount_column": "Monto",
        "destination_column": "Destino",
        "profit_loss_column": "G/P",
        "profit_loss_pct_column": "G/P %",
        
        # Destinos
        "personal_withdrawal": "Retiro Personal",
        "reinvestment": "Reinversion",
        "not_operated": "No Operado",
        
        # Panel de resumen
        "weekly_summary_panel": "📊 Resumen Semanal",
        "total_week": "Total Semana:",
        "performance": "Rendimiento:",
        "ai_advice": "💡 Consejo AI",
        "daily_advice_title": "Consejo del día",
        "days_label": "Días: +{positive} / -{negative}",
        "capital_initial": "Capital Inicial:",
        "current_balance": "Saldo Actual:",
        "total_profit_loss": "Ganancia/Pérdida Total:",
        "profit_loss_percentage": "Porcentaje G/P:",
        
        # Gráfico
        "weekly_chart_title": "📊 Gráfico de Rendimiento Semanal",
        "profit_loss_chart": "Gráfico de Ganancias/Pérdidas",
        "daily_performance_title": "Rendimiento Diario de Trading",
        "weekly_performance_title": "Rendimiento Semanal de Trading",
        "days_of_week_label": "Días de la Semana",
        "amount_axis_label": "Monto ($)",
        "profit_loss_axis_label": "Ganancia/Pérdida ($)",
        "average_label": "Promedio:",
        "legend_gain": "Ganancia",
        "legend_loss": "Pérdida",
        "legend_neutral": "Neutro",
        "legend_gain_withdrawal": "Ganancia (Retiro)",
        "legend_gain_reinvestment": "Ganancia (Reinversión)",
        "chart_error_update": "Error al actualizar el gráfico",
        "chart_error_load": "Error al cargar gráfico",
        
        # Exportación
        "export_success": "Exportación exitosa",
        "export_error": "Error al exportar",
        "export_completed": "Los datos se han exportado correctamente",
        "export_failed": "No se pudieron exportar los datos",
        "export_save_title": "Guardar archivo de exportación",
        "selected_file": "Archivo seleccionado",
        "starting_export": "Iniciando exportación...",
        "open_file_location_question": "¿Deseas abrir la ubicación del archivo?",
        "no_data_to_export": "No hay datos para exportar",
        "format_label": "Formato:",
        "format_excel_recommended": "Excel (.xlsx) - Recomendado",
        "format_csv_compatible": "CSV (.csv) - Compatible con todos",
        "format_json_developers": "JSON (.json) - Para desarrolladores",
        "include_charts_excel": "Incluir gráficos en Excel (si es posible)",
        "include_detailed_summary": "Incluir resumen detallado",
        "preview_title": "👁️ Vista Previa",
        "select_file": "Seleccionar archivo",
        
        # Mensajes generales
        "initial_data_loaded_db": "Datos iniciales cargados desde base de datos",
        "no_previous_data_new_week": "No hay datos previos, comenzando con semana nueva",
        "confirm_close_title": "Confirmar cierre",
        "close_anyway_question": "¿Desea cerrar de todos modos?",
        
        # Mensajes de archivo
        "save_success": "Datos guardados correctamente",
        "load_success": "Datos cargados correctamente",
        "save_error": "Error al guardar",
        "load_error": "Error al cargar",
        "file_not_found": "Archivo no encontrado",
        "invalid_format": "Formato de archivo inválido",
        
        # Mensajes de validación
        "invalid_amount": "Monto inválido",
        "invalid_capital": "Capital inválido",
        "capital_required": "Debe establecer un capital inicial",
        "saving": "Guardando...",
        
        # Días de la semana
        "monday": "Lunes",
        "tuesday": "Martes",
        "wednesday": "Miércoles",
        "thursday": "Jueves",
        "friday": "Viernes",
        "saturday": "Sábado",
        "sunday": "Domingo",
        
        # Consejos AI
        "ai_analysis_title": "📈 Análisis de Rendimiento AI",
        "ai_recommendation": "💡 Recomendación AI",
        "ai_risk_alert": "⚠️ Alerta de Riesgo",
        "insights": "INSIGHTS",
        "recommendations": "RECOMENDACIONES",
        "risk_assessment": "EVALUACIÓN DE RIESGO",
        "rating": "CALIFICACIÓN",
        "no_analysis": "Sin análisis disponible",
        
        # Otros
        "week": "Semana",
        "loading": "Cargando...",
        "error": "Error",
        "success": "Éxito",
        "warning": "Advertencia",
        "information": "Información",
        "confirm": "Confirmar",
        "operation_completed": "Operación completada",
        "operation_failed": "Operación fallida",
        
        # Diálogo de cargar semana guardada
        "load_week_dialog_title": "Seleccionar semana guardada",
        "load_week_action": "Cargar semana",
        "delete_week": "Borrar semana",
        "no_saved_weeks": "No hay semanas guardadas",
        "select_week_first": "Seleccione una semana primero",
        "confirm_delete_week_title": "Confirmar eliminación",
        "confirm_delete_week_message": "¿Desea borrar la semana seleccionada?",
        "week_label": "Semana",
        "delete_success": "Semana borrada correctamente",
        "delete_error": "Error al borrar la semana"
    },
    "en": {
        # Window titles
        "app_title": f"W-T-F (Weekend Trading Finance) Trading Manager v{APP_VERSION}",
        "capital_dialog_title": "Set Initial Capital",
        "export_dialog_title": "Export Data",
        "about_title": "About W-T-F Trading Manager",
        "instructions_title": "Usage Instructions",
        "save_week_title": "Save Week",
        "load_week_title": "Load Week",
        
        # Main menu
        "menu_file": "📁 File",
        "menu_view": "👁️ View",
        "menu_assistant": "🧭 Assistant",
        "menu_export": "📊 Export",
        "menu_help": "❓ Help",
        "menu_language": "🌍 Language",
        
        # Menu actions
        "save_week": "💾 Save Week",
        "load_week": "📂 Load Week",
        "load_from_db": "🗄️ Load from Database",
        "set_capital": "💰 Set Initial Capital",
        "exit": "🚪 Exit",
        "dark_mode": "🌙 Dark Mode",
        "toggle_legend": "👁️ Show Legend",
        "daily_advice": "📌 Show daily advice",
        "weekly_summary": "🗓️ Weekly summary",
        "start_new_week_reset": "🆕 Start new week (reset data)",
        "export_excel": "📈 Export to Excel",
        "export_csv": "📋 Export to CSV",
        "export_json": "📄 Export to JSON",
        "about": "ℹ️ About",
        "instructions": "📖 Instructions",
        "capital_edit_mode": "💹 Capital edit mode",
        
        # Menu StatusTips
        "status_save_week": "Save current week's data",
        "status_load_week": "Load data from file",
        "status_load_db": "Load saved data from database",
        "status_set_capital": "Set the week's initial capital",
        "status_exit": "Exit the application",
        "status_dark_mode": "Toggle dark mode",
        "status_toggle_legend": "Show or hide the chart legend",
        "status_daily_advice": "View recommendations for the current day",
        "status_weekly_summary": "Show summary with withdrawal and reinvestment suggestion",
        "status_start_new_week_reset": "Create a new week with zeroed data and updated capital",
        "status_export_excel": "Export data to Excel (.xlsx)",
        "status_export_csv": "Export data to CSV",
        "status_export_json": "Export data to JSON",
        "status_about": "Information about the application",
        "status_instructions": "View usage instructions",
        "status_capital_edit_mode": "Edit day amount by initial/current capital",
        
        # Languages
        "spanish": "Spanish",
        "english": "English",
        
        # Buttons and general labels
        "accept": "Accept",
        "cancel": "Cancel",
        "save": "Save",
        "close": "Close",
        "yes": "Yes",
        "no": "No",
        
        # Capital dialog
        "capital_label": "Initial Capital:",
        "capital_tooltip": "Enter initial capital for the week",
        "capital_placeholder": "e.g.: 100.00",
        "day_capital_dialog_title": "Enter day capital",
        "initial_capital": "Initial capital",
        "current_capital": "Current capital",
        
        # Trading table
        "day_column": "Day",
        "date_column": "Date",
        "type_column": "Type",
        "comments_column": "Comments",
        "amount_column": "Amount",
        "destination_column": "Destination",
        "profit_loss_column": "P/L",
        "profit_loss_pct_column": "P/L %",
        
        # Destinations
        "personal_withdrawal": "Personal Withdrawal",
        "reinvestment": "Reinvestment",
        "not_operated": "Not Operated",
        
        # Summary panel
        "weekly_summary_panel": "📊 Weekly Summary",
        "total_week": "Weekly Total:",
        "performance": "Performance:",
        "ai_advice": "💡 AI Advice",
        "daily_advice_title": "Daily Advice",
        "days_label": "Days: +{positive} / -{negative}",
        "capital_initial": "Initial Capital:",
        "current_balance": "Current Balance:",
        "total_profit_loss": "Total Profit/Loss:",
        "profit_loss_percentage": "P/L Percentage:",
        
        # Chart
        "weekly_chart_title": "📊 Weekly Performance Chart",
        "profit_loss_chart": "Profit/Loss Chart",
        "daily_performance_title": "Daily Trading Performance",
        "weekly_performance_title": "Weekly Trading Performance",
        "days_of_week_label": "Days of the Week",
        "amount_axis_label": "Amount ($)",
        "profit_loss_axis_label": "Profit/Loss ($)",
        "average_label": "Average:",
        "legend_gain": "Gain",
        "legend_loss": "Loss",
        "legend_neutral": "Neutral",
        "legend_gain_withdrawal": "Gain (Withdrawal)",
        "legend_gain_reinvestment": "Gain (Reinvestment)",
        "chart_error_update": "Error updating chart",
        "chart_error_load": "Error loading chart",
        
        # Export
        "export_success": "Export successful",
        "export_error": "Export error",
        "export_completed": "Data has been exported successfully",
        "export_failed": "Could not export data",
        "export_save_title": "Save export file",
        "selected_file": "Selected file",
        "starting_export": "Starting export...",
        "open_file_location_question": "Do you want to open the file location?",
        "no_data_to_export": "No data to export",
        "format_label": "Format:",
        "format_excel_recommended": "Excel (.xlsx) - Recommended",
        "format_csv_compatible": "CSV (.csv) - Compatible everywhere",
        "format_json_developers": "JSON (.json) - For developers",
        "include_charts_excel": "Include charts in Excel (if possible)",
        "include_detailed_summary": "Include detailed summary",
        "preview_title": "👁️ Preview",
        "select_file": "Select file",
        
        # General messages
        "initial_data_loaded_db": "Initial data loaded from database",
        "no_previous_data_new_week": "No previous data, starting a new week",
        "confirm_close_title": "Confirm close",
        "close_anyway_question": "Do you want to close anyway?",
        
        # File messages
        "save_success": "Data saved successfully",
        "load_success": "Data loaded successfully",
        "save_error": "Save error",
        "load_error": "Load error",
        "file_not_found": "File not found",
        "invalid_format": "Invalid file format",
        
        # Validation messages
        "invalid_amount": "Invalid amount",
        "invalid_capital": "Invalid capital",
        "capital_required": "You must set an initial capital",
        "saving": "Saving...",
        
        # Days of the week
        "monday": "Monday",
        "tuesday": "Tuesday",
        "wednesday": "Wednesday",
        "thursday": "Thursday",
        "friday": "Friday",
        "saturday": "Saturday",
        "sunday": "Sunday",
        
        # AI Advice
        "ai_analysis_title": "📈 AI Performance Analysis",
        "ai_recommendation": "💡 AI Recommendation",
        "ai_risk_alert": "⚠️ Risk Alert",
        "insights": "INSIGHTS",
        "recommendations": "RECOMMENDATIONS",
        "risk_assessment": "RISK ASSESSMENT",
        "rating": "RATING",
        "no_analysis": "No analysis available",
        
        # Other
        "week": "Week",
        "loading": "Loading...",
        "error": "Error",
        "success": "Success",
        "warning": "Warning",
        "information": "Information",
        "confirm": "Confirm",
        "operation_completed": "Operation completed",
        "operation_failed": "Operation failed",
        
        # Load saved week dialog
        "load_week_dialog_title": "Select saved week",
        "load_week_action": "Load week",
        "delete_week": "Delete week",
        "no_saved_weeks": "No saved weeks",
        "select_week_first": "Select a week first",
        "confirm_delete_week_title": "Confirm deletion",
        "confirm_delete_week_message": "Do you want to delete the selected week?",
        "week_label": "Week",
        "delete_success": "Week deleted successfully",
        "delete_error": "Error deleting week"
    }
}


# Textos de la versión 3.0 (configuración, gráfico 3D e imagen de resultado).
# En español se usan los valores por defecto definidos junto a cada tr().
TRANSLATIONS['en'].update({
    'image_engine': 'Image engine',
    'image_engine_desc': 'Local: free, offline and the numbers are always exact.',
    'engine_ai': 'AI · Pollinations',
    'engine_local': 'Local · exact values (Pillow)',
    'ref_values_hint': 'Values written on the reference image (the AI replaces them with the new ones):',
    'edit_prompt_hint': 'The edit prompt is built automatically from the values that change.',
    'try_local': 'Generate locally with exact values',
    'local_no_prompt': 'Local engine: the card is drawn with Pillow, no prompt.',
    'no_changes': 'No changes: the values match the reference image.',
    'local_exact': 'Local · exact',
    'mode_edit': 'Edit',
    'mode_new': 'From scratch',
    'session_details': 'Session details',
    'session_details_hint': 'Shown on the chart and on the result image.',
    'currency_pair': 'Currency pair',
    'session_duration': 'Duration',
    'chart_mode_tooltip': 'Switch chart view (drag to rotate in 3D)',
    'cumulative_label': 'Cumulative',
    'image_placeholder': 'Your result card will appear here.\nAdjust the data and press “Generate image”.',
    'result_image_title': 'Result image',
    'result_image_subtitle': 'Edit your card with AI (Pollinations) or create it locally with exact values.',
    'api_key_missing': 'Pollinations API key is missing (POLLINATIONS_API_KEY in .env).',
    'ref_image_missing': 'The reference image (base card) is missing.',
    'configure': 'Configure',
    'day_text_tooltip': 'Day text exactly as it will appear in the image',
    'trading_day': 'Trading day',
    'day_in_image': 'Day (in image)',
    'result_label': 'Result',
    'save_to_day': 'Save pair, duration and result to the day',
    'show_prompt': 'Show final prompt',
    'generate_image': 'Generate image',
    'preview': 'Preview',
    'save_as': 'Save as…',
    'copy': 'Copy',
    'open_folder': 'Open folder',
    'generating': 'Generating…',
    'regenerate': 'Generate again',
    'image_decode_error': 'Could not read the received image.',
    'image_ready': 'Image ready',
    'image_error': 'Could not generate the image',
    'copied': 'Image copied to clipboard',
    'settings_title': 'Settings',
    'save_changes': 'Save changes',
    'settings_appearance': 'Appearance',
    'settings_appearance_desc': 'Theme, language and chart style. Changes apply instantly.',
    'theme_section': 'Theme & language',
    'dark_mode_desc': 'Fintech-style dark interface.',
    'language_desc': 'Some texts update when this window is reopened.',
    'chart_section': 'Chart',
    'chart_3d': '3D volumetric (drag to rotate)',
    'chart_2d': '2D neon glow',
    'chart_mode': 'Chart view',
    'chart_animations': 'Animations',
    'chart_animations_desc': 'Growing bars and camera sweep on update.',
    'settings_trading': 'Trading',
    'settings_trading_desc': 'Capital, edit mode and weekly cycle.',
    'capital_section': 'Capital',
    'edit': 'Edit',
    'show': 'Show',
    'week_section': 'Week',
    'start': 'Start',
    'settings_data': 'Data & export',
    'settings_data_desc': 'Save, load and export your weeks.',
    'open': 'Open',
    'export': 'Export',
    'save': 'Save',
    'settings_ai': 'AI · Result image',
    'settings_ai_desc': 'Edit your reference card with Pollinations (FLUX.1 Kontext) or render it locally with exact values.',
    'api_key': 'Pollinations API key',
    'ref_image': 'Reference image',
    'ref_image_hint': 'The base card the AI edits. If it does not exist, the card is generated from scratch with the generation prompt.',
    'choose_image': 'Choose image…',
    'missing': 'Not found',
    'get_api_key': 'Get key',
    'test_connection': 'Test connection',
    'api_key_note': 'Stored in the app .env file as POLLINATIONS_API_KEY (never in code or the database). Use a secret sk_ key.',
    'model_section': 'Model & image',
    'model': 'Model',
    'aspect_ratio': 'Aspect ratio',
    'default_pair': 'Default pair',
    'default_duration': 'Default duration',
    'output_folder': 'Images folder',
    'autosave_images': 'Automatically save every generated image',
    'prompt_template': 'Generation-from-scratch prompt',
    'prompt_hint': 'Used only when there is no reference image. Variables: ',
    'restore_default': 'Restore default',
    'settings_about': 'Help',
    'settings_about_desc': 'App information and usage guide.',
    'shortcuts': 'Keyboard shortcuts',
    'generate_result_image': 'Generate result image',
    'language_applied': 'Language applied',
    'testing': 'Testing…',
    'connection_ok': 'Connection OK',
    'models': 'models',
    'session_column': 'Session (pair · duration)',
    'amount_tooltip': "Double-click to edit the day's profit/loss",
    'add_session': 'Add',
    'session_tooltip': 'Double-click to note currency pair and duration',
    'edit_amount': 'Edit result',
    'week_results': 'Week results',
    'table_hint': 'Double-click to edit · right-click for more options',
    'welcome_title': 'Welcome to W-T-F!',
    'welcome_capital_desc': "To get started, enter the capital you're starting this week with. "
                            "We'll use it to calculate your balance and performance.",
    'use_default_capital': 'Use default $100',
    'start_week': 'Start week',
    'save_capital': 'Save capital',
    'initial_capital_caps': 'INITIAL CAPITAL',
    'capital_hint': 'You can change it anytime in Settings → Trading.',
    'instructions_subtitle': 'Everything you need to log and analyze your week.',
    'got_it': 'Got it',
    'made_by': 'Made by',
    'about_description': 'Application to manage and analyze weekly trading performance, '
                         'with AI analysis, data persistence and enhanced visualizations.',
    'features': 'Features',
    'technologies': 'Technologies',
    'follow_me': 'Follow me on',
    'follow_me_desc': 'Tutorials, news and trading results.',
    'view_on_github': 'View project on GitHub',
    'week_profitable': 'Profitable week',
    'week_challenging': 'Challenging week',
    'week_flat': 'Flat week',
    'win_rate': 'Win rate',
    'win_days': '{wins} of {total} days',
    'best_day': 'Best day',
    'worst_day': 'Worst day',
    'daily_average': 'Daily average',
    'per_traded_day': 'per traded day',
    'daily_breakdown': 'Daily breakdown',
    'no_session': 'No session',
    'money_plan': 'Plan for next week',
    'withdraw_30': 'Recommended withdrawal (30%)',
    'suggested_reinvestment': 'Suggested reinvestment',
    'next_week_capital': 'Next week capital',
    'tip_profit': 'Lock in part of the profit and reinvest the rest: compounding does the work.',
    'tip_loss': 'Review your worst trades, lower risk per trade and protect your capital.',
    'tip_flat': 'Document your best and worst trades to learn faster.',
    'copy_summary': 'Copy summary',
    'copied': 'Copied',
    'weeks_folder': 'Weeks folder',
    'weeks_folder_desc': 'Your weeks are saved here (Ctrl+S) and loaded from here (Ctrl+O). '
                         'You can use a synced folder (OneDrive, Google Drive...).',
    'change_folder': 'Change folder',
    'open_folder': 'Open folder',
    'restore_default': 'Reset',
    'copy_weeks_question': 'Copy the {n} saved weeks to the new folder?\nOriginal files are not deleted.',
    'weeks_folder_changed': 'Weeks folder updated',
    'other_folder': 'Other folder',
    'other_folder_tip': 'Load weeks from another folder (this time only)',
    'export_subtitle': 'Download your results to analyze or share them.',
    'fmt_excel_desc': 'Formulas and charts',
    'fmt_csv_desc': 'Universal table',
    'fmt_json_desc': 'For developers',
    'open_after_export_desc': 'Opens with the default app.',
    'export_options': 'What and how to export',
    'current_week': 'Current week',
    'all_weeks': 'Full history',
    'include_charts': 'Include charts',
    'include_charts_desc': 'Daily results and balance evolution (Excel only).',
    'include_summary_desc': 'Capital, performance, win rate, withdrawal and next week capital.',
    'csv_regional': 'CSV for Spanish-locale Excel',
    'csv_regional_desc': 'Uses semicolons (;) and decimal commas so Excel splits it into columns.',
    'open_after_export': 'Open the file when done',
    'destination': 'Destination',
    'change': 'Change',
    'open_file': 'Open file',
    'export': 'Export',
    'export_again': 'Export again',
    'overwrite_question': 'The file already exists. Replace it?',
    'file_in_use': 'Could not write: is the file open in Excel?',
    'showing_first': 'showing the first {n}',
    'rows_word': 'rows',
    'weeks_word': 'weeks',
    'history': 'History',
    'detail': 'Detail',
    'date_column': 'Date',
    'result_column': 'Result',
    'cumulative_column': 'Cumulative',
    'balance_column': 'Balance',
    'final_balance': 'Final balance',
    'balance_evolution': 'Balance evolution',
    'weekly_evolution': 'Weekly evolution',
})


# Textos de la versión 3.2 (importación y análisis, capital del día).
TRANSLATIONS['en'].update({
    'settings_data_desc': 'Save, load, export and import your weeks.',
    'import_analyze': 'Import',
    'import_title': 'Import & analyze trades',
    'import_subtitle': 'Excel or CSV from your broker or exported from W-T-F: averages, charts and stats instantly.',
    'import_row_desc': 'Excel or CSV from the broker or W-T-F: averages, charts and statistics.',
    'open_files': 'Open files…',
    'drop_title': 'Drop your files here',
    'drop_desc': 'or press the button to choose them. You can combine several files: '
                 'repeated trades (same ID) are counted once.',
    'fmt_broker': 'Broker history',
    'fmt_broker_desc': 'One row per trade: asset, time, direction, amount and payout (e.g. Quotex).',
    'fmt_wtf': 'W-T-F export',
    'fmt_wtf_desc': 'Excel, CSV or JSON exported from this app, or a saved week.',
    'fmt_generic': 'Your own table',
    'fmt_generic_desc': 'Any sheet with a date column and a result column.',
    'tab_summary': 'Summary',
    'tab_charts': 'Charts',
    'tab_days': 'By day',
    'tab_operations': 'Trades',
    'capital_ref_tip': 'Reference capital for balance, growth % and drawdown %',
    'period': 'Period',
    'asset': 'Asset',
    'capital_short': 'Capital',
    'import_hint': 'Formats: .xlsx, .xls, .csv, .json',
    'export_analysis': 'Export analysis',
    'export_analysis_tip': 'Save KPIs, daily summary, assets and trades to Excel (with charts)',
    'apply_to_week': 'Load into week',
    'apply_to_week_tip': "Writes each day's net result into the current week ({w})",
    'apply_to_week_none': 'No trades from the current week ({w}) with these filters',
    'apply_to_week_confirm': 'The net result of these days will be written into the current week:',
    'replaces': 'was',
    'week_updated_from_import': 'Week updated with {n} imported days',
    'import_errors': 'Some files could not be read:',
    'operations_word': 'trades',
    'days_word': 'days',
    'trading_days_word': 'trading days',
    'all_period': 'Whole period',
    'last_7_days': 'Last 7 days',
    'last_30_days': 'Last 30 days',
    'all_assets': 'All',
    'no_rows_filter': 'No trades match these filters.',
    'net_result': 'Net result',
    'over_capital': 'on',
    'over_staked': 'on amount invested',
    'avg_per_op': 'Average per trade',
    'green_days': 'Green days',
    'won_short': 'won',
    'lost_short': 'lost',
    'draw_short': 'draw',
    'profit_factor': 'Profit factor',
    'profit_factor_desc': 'gains ÷ losses',
    'per_day': 'per day',
    'in_green': 'green',
    'avg_win': 'Average win',
    'per_winner': 'per winning trade',
    'per_green_day': 'per green day',
    'avg_loss': 'Average loss',
    'per_loser': 'per losing trade',
    'per_red_day': 'per red day',
    'max_drawdown': 'Max drawdown',
    'from_peak': 'from peak',
    'drawdown_desc': 'largest cumulative drop',
    'streaks': 'Longest streaks',
    'streaks_desc': 'wins / losses in a row',
    'total_staked': 'Total invested',
    'volume': 'traded volume',
    'avg_stake': 'Average amount',
    'per_operation': 'per trade',
    'avg_payout': 'Average payout',
    'breakeven': 'Break-even rate',
    'cumulative_evolution': 'Cumulative result',
    'hover_hint': 'Hover the chart to inspect each point',
    'operation': 'trade',
    'day_word': 'day',
    'above_breakeven': 'above',
    'below_breakeven': 'below',
    'insight_breakeven': 'With an average payout of {p:.1f}% you need a win rate of at least '
                         '<b>{be:.1f}%</b> to break even; you are <b>{v}</b> at {wr:.1f}%.',
    'insight_expectancy': 'Expectancy per trade: {e} (average win {w} · average loss {l}).',
    'insight_hours': 'Best time window: <b>{b:02d}:00–{b1:02d}:00</b> ({bn}, {bw:.0f}% win rate). '
                     'Worst: <b>{w:02d}:00–{w1:02d}:00</b> ({wn}).',
    'insight_direction': 'By direction — {parts}.',
    'insight_stake': 'With amounts of <b>{s}</b> the result is {n} ({wr:.0f}% win rate): '
                     'review your risk management when increasing the stake.',
    'insight_days': '{g} of {t} days in the green · daily average {a}.',
    'insight_weekday': 'Your best weekday is <b>{d}</b> ({n} in total).',
    'insight_asset': 'Most profitable asset: <b>{a}</b> ({n} in {o} {u}).',
    'insight_drawdown': 'The largest drop from a peak was <b>{d}</b>; longest losing streak: {l} in a row.',
    'chart_daily': 'Result by day',
    'chart_outcomes': 'Won / lost',
    'chart_hours': 'Result by hour',
    'chart_weekday': 'Result by weekday',
    'chart_assets': 'Result by asset',
    'chart_direction': 'Result by direction',
    'chart_stake': 'Result by amount invested',
    'chart_distribution': 'Result distribution',
    'win_rate_short': 'win rate',
    'average': 'Average',
    'outcome_win': 'Won',
    'outcome_loss': 'Lost',
    'outcome_draw': 'Draw',
    'outcome_wins': 'Won',
    'outcome_losses': 'Lost',
    'outcome_draws': 'Draws',
    'dir_up': 'Up',
    'dir_down': 'Down',
    'ops_short': 'Trades',
    'won_lost': 'Won / Lost',
    'staked': 'Invested',
    'net': 'Net',
    'by_asset': 'By asset',
    'direction': 'Direction',
    'stake': 'Amount',
    'payout': 'Payout',
    'status_column': 'Status',
    'day_capital_title': 'Day capital',
    'day_capital_desc': 'Enter how much you started and finished the day with: '
                        'we calculate the profit or loss for you.',
    'day_start_ref': 'Start of day',
    'week_capital_ref': 'Weekly capital',
    'start_from': 'Start from:',
    'final_capital_caps': 'FINAL CAPITAL',
    'day_result_caps': "DAY'S RESULT",
    'day_capital_hint': 'Enter saves · Esc cancels. You can turn this mode off in Settings → Trading.',
    'save_result': 'Save result',
    'saved_before': 'Saved: {v}',
})

# Idioma actual
current_language = "es"

def set_language(lang):
    """Cambiar el idioma actual"""
    global current_language
    if lang in TRANSLATIONS:
        current_language = lang

def tr(key, default=None):
    """Obtener traducción para una clave"""
    return TRANSLATIONS[current_language].get(key, default or key)

def get_available_languages():
    """Obtener lista de idiomas disponibles"""
    return list(TRANSLATIONS.keys())
